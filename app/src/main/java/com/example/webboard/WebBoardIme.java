...
public class WebBoardIme extends InputMethodService {
    ...
    @Override public void onStartInput(android.view.inputmethod.EditorInfo attribute, boolean restarting) {\n        super.onStartInput(attribute, restarting);\n        addCurrentClipboard();\n    }\n\n    @Override public boolean onEvaluateFullscreenMode() {
        // Let Android use the IME as a fullscreen keyboard where supported.
        return true;
    }
    ...
    @Override public View onCreateInputView() {
        ...
        root.setPadding(dp(5),dp(5),dp(5),dp(4));
        applyBackground(root);

        root.addView(buildBrowserBar(),new LinearLayout.LayoutParams(-1,dp(46)));
        ...
        web.setWebViewClient(new WebViewClient(){
            @Override public void onPageFinished(WebView v,String u){super.onPageFinished(v,u);injectFocusBridge();}
        });
        web.loadUrl("https://www.google.com/");
        root.addView(web,new LinearLayout.LayoutParams(-1,0,0.9f));

        keys=new LinearLayout(this);
        keys.setOrientation(LinearLayout.VERTICAL);
        root.addView(keys,new LinearLayout.LayoutParams(-1,0,1.1f));
        buildKeys();
        return root;
    }
